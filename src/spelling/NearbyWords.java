...
public class NearbyWords implements SpellingSuggest {
    ...
    public void insertions(String s, List<String> currentList, boolean wordsOnly) {
        for (int insertIndex = 0; insertIndex < s.length() + 1; ++insertIndex) {
            for (int charCode = 'a'; charCode <= (int) 'z'; ++charCode) {

                StringBuilder sb = new StringBuilder(s);

                if (insertIndex >= s.length()) sb.append((char) charCode);
                else sb.setCharAt(insertIndex, (char) charCode);

                for (int originalIndex = insertIndex; originalIndex < s.length(); ++originalIndex) {
                    if (originalIndex + 1 >= s.length()) sb.append(s.charAt(originalIndex));
                    else sb.setCharAt(originalIndex + 1, s.charAt(originalIndex));
                }
    ...
    public void deletions(String s, List<String> currentList, boolean wordsOnly) {
        for (int deleteIndex = 0; deleteIndex < s.length(); ++deleteIndex) {
            StringBuilder sb = new StringBuilder();

            int originalIndex = 0;
            while (originalIndex < deleteIndex) {
                sb.append(s.charAt(originalIndex));
                ++originalIndex;
            }
    ...
    public List<String> suggestions(String word, int numSuggestions) {
        ...
        while (!queue.isEmpty() && retList.size() < numSuggestions) {
            for (String s : distanceOne(queue.poll(), true)) {
